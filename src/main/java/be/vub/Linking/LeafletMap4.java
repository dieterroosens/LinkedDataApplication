...
@WebServlet(name = "LeafletMap4Servlet", urlPatterns = {"/map4","/leafletmap4"})
public class LeafletMap4 extends HttpServlet {
    ...
    private String getAllParametersForSummary(HttpServletRequest request, HttpServletResponse response)throws IOException{
        ...
        for(String parameter : parameters.keySet()) {
            ...
            if(parameter.toLowerCase().startsWith("maplayer")){
                ;
            }
            else{
                    ...
                    for (String value : values) {
                        ...
                        else
                        {
                            ...
                            if (value.contains("@") || value.contains("^^")) {
                                ...
                                if(parameter.toLowerCase().startsWith("label")){
                                    sb.append(prefix.adaptPredicate(parameter) + " '" + value + "'. ");
                                }
                                else{
                                    sb.append(prefix.adaptPredicate(parameter) + " <" + value + ">. ");
    ...
    private void makeMap(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String mapLayer=request.getParameter("MapLayer");
        if(mapLayer!=null && mapLayer.equals("TerrainMap"))
        {
            javascriptIncluder(request, response, "leaflet/createTerrainMapElement.js");
        }
        else if(mapLayer!=null && mapLayer.equals("SatelliteMap"))
        {
            javascriptIncluder(request, response, "leaflet/createSatelliteMapElement.js");
        }
        else if(mapLayer!=null && mapLayer.equals("WorldTopoMap"))
        {
            javascriptIncluder(request, response, "leaflet/createWorldTopoMapElement.js");
        }
        else if(mapLayer!=null && mapLayer.equals("HikeBikeMap"))
        {
            javascriptIncluder(request, response, "leaflet/createHikeBikeMapElement.js");
        }
