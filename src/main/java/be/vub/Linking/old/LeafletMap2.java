...
@WebServlet(name = "LeafletMap2Servlet", urlPatterns = {"/map2","/leafletmap2"})
public class LeafletMap2 extends HttpServlet {
...
/*    private void addPoints(PrintWriter out){
        ...
        List<MapElement> mapElements = callRDF.doCCallAll();
        for(MapElement mapElement: mapElements){
            out.println("<script>\n");
            *//*out.println("L.polygon([\n" +
                    "        [51.509, -0.08],\n" +
                    "        [51.503, -0.06],\n" +
                    "        [51.51, -0.047]\n" +
                    "    ]).addTo(mymap).bindPopup(\"I am a polygon.\");");*//*
                    if(mapElement.getPoints().size()==1) {
                        out.println("    L.marker(" + mapElement.giveLeafletPoint() + ").addTo(mymap)\n");
                    }
                    else {
                        out.println("    L.polygon(" + mapElement.giveAllPoints() + ").addTo(mymap)\n");
                    }
                    out.println("        .bindPopup(\""+mapElement.getPopupText()+"\").openPopup();\n"+
